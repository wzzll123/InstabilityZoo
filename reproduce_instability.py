...
def load_indices():
    with open(os.path.join(indices_path), 'r') as f:
        logged_indices = [list(map(int, line.strip().split(','))) for line in f] 
    tmp_indices = []
    for row in logged_indices:
...
def get_batch(split):
    ...
    if split == 'train':
        ...
        global cur_indices_index
        indices = logged_indices[cur_indices_index]
        cur_indices_index += 1
        ix = torch.tensor(indices, dtype=torch.int64)
    else:
...
# Extract r[G, t]
if log_max_logits:
    attn_layer_text = [0,1,2,3,4, len(raw_model.transformer.h)-1]
    attn_logits_collected_layers = [raw_model.transformer.h[0], raw_model.transformer.h[1],raw_model.transformer.h[2], raw_model.transformer.h[3],raw_model.transformer.h[4], raw_model.transformer.h[-1]]
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if log_adam_m_v:
            ...
            for layer in collected_layers:
                # Function to get the attribute name of a layer
                def get_layer_name(model, layer):
                    for name, module in model.named_modules():
                        if module is layer:
                            return name
                    return "unknown_layer"
                layer_name = get_layer_name(raw_model, layer)
                r_values[iter_num][layer] = []
                for p in layer.parameters():  # First transformer layer
                    if p in optimizer.state and 'exp_avg' in optimizer.state[p] and 'exp_avg_sq' in optimizer.state[p]:
                        m_t = optimizer.state[p]['exp_avg']
                        v_t = optimizer.state[p]['exp_avg_sq']
                        r = (m_t / (torch.sqrt(v_t))).flatten()

                        # Randomly sample indices on the GPU
                        sample_size = max(1, int(0.01 * r.numel()))
                        sampled_indices = torch.randint(0, r.numel(), (sample_size,), device=r.device)
